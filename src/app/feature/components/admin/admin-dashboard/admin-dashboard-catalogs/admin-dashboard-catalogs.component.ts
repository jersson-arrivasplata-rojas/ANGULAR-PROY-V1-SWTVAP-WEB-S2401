...
export class AdminDashboardCatalogsComponent implements OnInit {
  ...
  public idGeneral = '#STORE-DASHBOARD-CATALOGS';
...
//Emojis
  ...
  ngOnInit(): void {
    Metodos.menuDashboardActive('catalogs');

    $.getScript(this.APP_URL+'assets/javascript/tours/dashboard/dashboard-catalog-tour.js');
  ...
  listOrderChanged($event){
    ...
    $.ajax({
      ...
      url: own.APP_URL_API + 'dashboard/catalogs/update-positions',//
    ...
    setTimeout(function(){
      ...
      for (let index = 0; index < emojisFirst.length; index++) {
        ...
        element.addEventListener('click', function($event){
          ...
          var text = $(own.idGeneral).find('.modal').find('.description-catalog').text();
          var emoji = $($event.target).text();
          $(own.idGeneral).find('.modal').find('.description-catalog').html(text + emoji)
        });
      }
      ...
      for (let index = 0; index < emojisSecond.length; index++) {
        ...
        element.addEventListener('click', function($event){
          console.log($($event.target));
          var text = $(own.idGeneral).find('.modal').find('.description-catalog-edit').text();
          var emoji = $($event.target).text();
          $(own.idGeneral).find('.modal').find('.description-catalog-edit').html(text + emoji)
        });
      }
    },1000);
//.emojiP-fourth,.emojiP-second,,.emojiP-three
  ...
  deleteModalCategory(self, id) {
    ...
    Swal.fire({
      ...
      text: "¡Si Eliminas la categoría, tambien se Eliminaran las Subcatalogs y Productos que le partenecen. !",
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        $.ajax({
          // la URL para la petición
          url: own.APP_URL_API + 'dashboard/catalogs/'+ id+'/'+own.store.stores_id,//
  ...
  editModalCategory(self, id) {
    ...
    $.ajax({
      // la URL para la petición
      url: own.APP_URL_API + 'dashboard/catalogs/' + id + '/edit',//ok
      ...
      success: function (data) {
        ...
        //console.log(data);
        var description = document.getElementById("description-catalog-edit");
        description.textContent = data['description'];
        // $("#editModal .card-img").attr('src','assets/images/users/catalogs/'+data['image']);
        //alert(data['status']);
        $(own.idGeneral).find("#editModal input[name=catalog_visible]").attr('value', data['status']); // aca seteo el valor al input
        var c = $(own.idGeneral).find("#editModal input[name=catalog_visible]").val(); // aca me imprime el valor que tiene ese input

        if (c == 1) {
          $(own.idGeneral).find("#editModal input[name=catalog_visible]").prop('checked', true);
        } else {
          $(own.idGeneral).find("#editModal input[name=catalog_visible]").prop('checked', false);
        }
        if (data['image'] == null || data['image'] == "") {
         // $("#editModal .card-img").attr('src', '../' + IMG_ASSETS);
        } else {
          /*this.getDataUri('assets/images/users/catalogs/' + data['image'], function (dataUri) {
            //  console.log(dataUri)
  ...
  getCategories() {
    ...
    Metodos.insertPreloadStoreRemoveHidden('PRELOAD-DASHBOARD-CATALOGS', this.getImagePrincipal(), this.store.stores_name);
  
    $.ajax({
      // la URL para la petición
      url: own.APP_URL_API + 'dashboard/catalogs',//ok
      ...
      complete: function (xhr, status) {
        ...
        Metodos.removeNodoPreloadHidden('PRELOAD-DASHBOARD-CATALOGS');
      }
    });
  }
  ...
  addModalCategory(){
    ...
    $(own.idGeneral).find('.alerts').empty();
        ...
        var description = document.getElementById("description-catalog").innerHTML;
        var status = [];

        $(own.idGeneral).find("#addModal input[name=catalog_visible]").each(function() {
       ...
       /* if (image == IMG_ASSETS) {
            ...
            image = $("#addModal [name=catalog_image]").attr('src');
            data.image = image;
        }*/
        $.ajax({
            // la URL para la petición
  ...
  updateModalCategory(){
    ...
    var description = document.getElementById("description-catalog-edit").innerHTML;
    ...
    $(own.idGeneral).find("#editModal input[name=catalog_visible]").each(function() {
        var val = this.type == "checkbox" ? +this.checked : this.value;
        status.push(val);
    });
    ...
    var image = $(own.idGeneral).find("#editModal [name=catalog_image]").attr('src');
    /*if (image == IMG_ASSETS) {
        data.image = '';
    } else {
        image = $("#editModal [name=catalog_image]").attr('src');
        data.image = image;
    }*/


    if (($(own.idGeneral).find('#editModal div[name="catalog_name"]').text()).length <= 0) {
        ...
        $.ajax({
            // la URL para la petición
            url: own.APP_URL_API + 'dashboard/catalogs/' + id,//ok
