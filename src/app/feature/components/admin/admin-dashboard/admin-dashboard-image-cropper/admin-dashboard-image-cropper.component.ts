...
import { CropperPosition, Dimensions, ImageCroppedEvent, ImageTransform, resizeCanvas } from 'ngx-image-cropper';
...
import { getTransformationsFromExifData, supportsAutomaticRotation } from 'src/Utils/exif.utils';
import { HammerStatic } from 'src/Utils/hammer.utils';
/*
...
@Component({
  selector: 'app-admin-dashboard-image-cropper',
  templateUrl: './admin-dashboard-image-cropper.component.html',
  styleUrls: ['./admin-dashboard-image-cropper.component.scss']
})
export class AdminDashboardImageCropperComponent implements OnChanges, OnInit {
  private Hammer: any = typeof window !== 'undefined'
  ...
  private moveStart: any;//MoveStart
  ...
  marginLeft: SafeStyle | string = '0px';
   maxSize: Dimensions;//@Input()
  imageVisible = false;
  moveTypes: any;//MoveTypes
  ...
  private keyboardMoveCropper(event) {
      ...
      const moveType = event.shiftKey ? "resize" : "move";//MoveTypes.Resize  //MoveTypes.Move
  ...
  startMove(event: any, moveType: any, position: string | null = null): void {
      if (this.moveStart && this.moveStart.active && this.moveStart.type === "pinch") {//Move.Pinch
          return;
      }
  ...
  moveImg(event: any): void {
      if (this.moveStart.active) {
          ...
          if (this.moveStart.type === "move") {//MoveTypes.Move
              this.move(event);
              this.checkCropperPosition(true);
          } else if (this.moveStart.type === "resize") {//MoveTypes.Resize
  ...
  onPinch(event: any) {
      if (this.moveStart.active) {
          ...
          if (this.moveStart.type === "pinch") {//MoveTypes.Pinch
              this.resize(event);
              this.checkCropperPosition(false);
          }
          this.cd.detectChanges();
