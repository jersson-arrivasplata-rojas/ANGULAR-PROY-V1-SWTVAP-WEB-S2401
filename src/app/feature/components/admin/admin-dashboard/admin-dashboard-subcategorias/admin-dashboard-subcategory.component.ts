...
@Component({
  selector: 'app-admin-dashboard-subcategory',
  templateUrl: './admin-dashboard-subcategory.component.html',
  styleUrls: ['./admin-dashboard-subcategory.component.css']
})
export class AdminDashboardSubcategoryComponent implements OnInit {
  public APP_URL = environment.appUrl;
  public APP_URL_API = environment.apiUrl;
  public idGeneral = '#STORE-DASHBOARD-SUBCATEGORIAS';
...
//Emojis
  ...
  ngOnInit(): void {
    Metodos.menuDashboardActive('subcategorias');
    $.getScript(this.APP_URL+'assets/javascript/tours/dashboard/dashboard-subcategoria-tour.js');
  ...
  listOrderChanged($event){
    ...
    $.ajax({
      ...
      url: own.APP_URL_API + 'dashboard/subcategories/update-positions',//

      data : { sorted:sortedData,stores_id:own.store.stores_id},
...
//.emojiP-fourth,.emojiP-second,,.emojiP-three
  ...
  deleteModalSubCategory(self, id,category_id) {

    let own = this;
    Swal.fire({
      title: '¿Seguro de eliminar?',
      text: "¡Si Eliminas la subcategoría, tambien se Eliminaran los Productos que le partenecen. !",
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        $.ajax({
          // la URL para la petición
          url: own.APP_URL_API + 'dashboard/subcategories/'+ id,//
  ...
  editModalSubCategory(self, id) {
    ...
    $.ajax({
      // la URL para la petición
      url: own.APP_URL_API + 'dashboard/subcategories/' + id + '/edit',//ok
      ...
      success: function (data) {
       // console.log(data)
        $(own.idGeneral).find("#editModal select[name=store_category_id]").val(data['category_id'])

        //console.log(data);
        var description = document.getElementById("description-category-edit");
        description.textContent = data['description'];
        // $("#editModal .card-img").attr('src','assets/images/users/subcategories/'+data['image']);
        //alert(data['status']);
        ...
        if (data['image'] == null || data['image'] == "") {
         // $("#editModal .card-img").attr('src', '../' + IMG_ASSETS);
          ...
          /*this.getDataUri('assets/images/users/subcategories/' + data['image'], function (dataUri) {
            //  console.log(dataUri)
            $("#editModal .card-img").attr('src', dataUri);
          });*/
        }
        //own.getSubCategories();
      },
  ...
  getSubCategories() {
    ...
    Metodos.insertPreloadStoreRemoveHidden('PRELOAD-DASHBOARD-SUBCATEGORIAS', this.getImagePrincipal(), this.store.stores_name);
  
    $.ajax({
      // la URL para la petición
      url: own.APP_URL_API + 'dashboard/subcategories',//ok
      ...
      complete: function (xhr, status) {
        ...
        Metodos.removeNodoPreloadHidden('PRELOAD-DASHBOARD-SUBCATEGORIAS');
      }
    });
  }
  addModalSubCategory(){
    ...
    $(own.idGeneral).find('.alerts').empty();
        ...
        var category_id = $(own.idGeneral).find("#addModal select[name=store_category_id]").val();

        var image = '';
        var data = { description: description, status: status.toString(),  image:null,store_id:own.store.stores_id,category_id:category_id  }; //image: image, 
       ...
       /* if (image == IMG_ASSETS) {
        ...
        $.ajax({
            ...
            success: function(json) {
                ...
                Swal.fire(
                    'Agregado!',
                    'La Subcategoria ha sido agregado satisfactoriamente.',
                    'success'
                )
  ...
  updateModalSubCategory(){
    ...
    var category_id = $(own.idGeneral).find("#editModal select[name=store_category_id]").val();
    ...
    var data = { description: description, status: status.toString(),image:null,store_id:own.store.stores_id,category_id:category_id };
    ...
    if (($(own.idGeneral).find('#editModal div[name="categoria_name"]').text()).length <= 0) {
        ...
        $.ajax({
            // la URL para la petición
            url: own.APP_URL_API + 'dashboard/subcategories/' + id,//ok
